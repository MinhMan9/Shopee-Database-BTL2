...
-- =============================================
-- [NEW] CÁC PROCEDURE CRUD CHO SHOP (Added on 2025-12-05)
-- =============================================
...
-- =============================================
-- 2. Procedure thêm sản phẩm mới (Dùng cho Import Excel)
-- =============================================
...
BEGIN
    ...
    -- 2. Insert into PRODUCT_VARIANT
    INSERT INTO PRODUCT_VARIANT (item_id, prod_name, prod_description, price, stock_quantity, status, total_sales, rating_avg, illustration_images)
    VALUES (@ItemID, @ProdName, @Description, @Price, @Stock, N'Đang bán', 0, 0, @ImageURL);
    
    SET @ProdID = SCOPE_IDENTITY();

    -- 3. Insert into PRODUCT_IMAGE (Primary Image)
    IF @ImageURL IS NOT NULL AND @ImageURL <> ''
    BEGIN
        INSERT INTO PRODUCT_IMAGE (prod_id, image_url, is_primary)
        VALUES (@ProdID, @ImageURL, 1);
    END
    
    SELECT @ProdID as NewProdID;
END;
GO


-- =============================================
-- 3. Procedure Xóa sản phẩm (Soft Delete - Chuyển trạng thái)
-- =============================================
...
BEGIN
    ...
    BEGIN
        -- Soft Delete: Cập nhật trạng thái thành 'Ngừng kinh doanh'
        UPDATE PRODUCT_VARIANT
        SET status = N'Ngừng kinh doanh'
        WHERE prod_id = @ProdID;
        
        PRINT 'Đã chuyển trạng thái sản phẩm sang Ngừng kinh doanh';
    END
    ELSE
    BEGIN
        THROW 51000, 'Sản phẩm không thuộc quyền quản lý của Shop hoặc không tồn tại.', 1;
    END
END;
GO
